...
clin_query <- GDCquery(project = "TCGA-SKCM",
                       ...
                       data.format = 'BCR Biotab') #Fill in correct command
...
clinical.BCRtab.all <- GDCprepare(clin_query) # Fill in correct command
...
```{r}
write.csv(clinic, "/Users/erosmendoza/Documents/qbio_490_erosmendoza/analysis_data/skcm_clinical_data.csv", row.names=FALSE)
```

To read the csv back into your environment as a dataframe:

```{r}
clinic <- read.csv("/Users/erosmendoza/Documents/qbio_490_erosmendoza/analysis_data/skcm_clinical_data.csv")
```
...
```{r Exercise 3.1}
# write code here
dim(clinic) # There are 470 rows and 65 columns.
class(clinic$gender) # $gender data type is character.
clinic$age_at_diagnosis[5] # The 5th patient is 74 yrs old at the time of diagnosis.
clinic$age_at_diagnosis[2] # 82 yrs old
clinic$last_contact_days_to[2] # 1865 days
365*82 # 29930 days
(365*82)+1865 # 31795 days
((365*82)+1865)/365 # The 2nd patient was 87 yrs old at their last follow up.
```
...
# create plot here
#help(plot)
x_variable <- clinic$age_at_diagnosis
y_variable <- clinic$weight_kg_at_diagnosis
plot(x_variable, y_variable, main = "Age vs Weight at Diagnosis", xlab = "Age", ylab = "Weight(kg)", col = "blue")
```
...
# create plot here
#help(boxplot)
x_variable <- clinic$gender
y_variable <- clinic$height_cm_at_diagnosis
y_variable <- as.numeric(y_variable)
#class(x_variable)
#class(y_variable)
boxplot(y_variable ~ x_variable, main = "Boxplot of Height by Gender", xlab = "Gender", ylab = "Height(cm)", notch = TRUE)
```
...
```{r}
cleaned_clinic$survival_time <- ifelse(cleaned_clinic$death_days_to %in% c("[Not Applicable]", "[Not Available]"), cleaned_clinic$last_contact_days_to, cleaned_clinic$death_days_to)
```
...
#class(cleaned_clinic$survival_time)
```
...
cleaned_clinic$death_event <- as.logical(cleaned_clinic$death_event)
```
...
# 5. What is the approx. 15-year survival rate for "Old" skin cancer patients?
  The approx. 15-year (5475 days) survival rate for "Old" skin cancer patients is 0.20.
# 6. What is the p-value? Is it statistically significant and what does that tell us about the relationship between age and skin cancer survival? 
  The low p-value of 0.002 in this case suggests that there is a statistically significant difference in the survival rates between the different age groups. This indicates that age is a significant factor affecting skin cancer survival.
